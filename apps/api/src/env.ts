...
export const env = {
	...
	S3_BUCKET_NAME: getEnvVariable("S3_BUCKET_NAME", "cossistant-dev"),
	S3_REGION: getEnvVariable("S3_REGION", "us-east-1"),
	...
	VAPID_SUBJECT: getEnvVariable(
		"VAPID_SUBJECT",
		"mailto:support@cossistant.com"
	),
	// AI Provider configuration
	// "openrouter" (default) or "openai-compatible" for custom endpoints like 方舟
	AI_PROVIDER: getEnvVariable("AI_PROVIDER", "openrouter"),
	// OpenAI-compatible API base URL (required when AI_PROVIDER=openai-compatible)
	OPENAI_BASE_URL: getEnvVariable("OPENAI_BASE_URL", ""),
	// OpenAI-compatible API key (required when AI_PROVIDER=openai-compatible)
	OPENAI_API_KEY: getEnvVariable("OPENAI_API_KEY", ""),
	// Model ID overrides for OpenAI-compatible providers (e.g. 方舟)
	AI_MODEL_DEFAULT: getEnvVariable("AI_MODEL_DEFAULT", ""),
	AI_MODEL_FAST: getEnvVariable("AI_MODEL_FAST", ""),
	AI_MODEL_SUMMARY: getEnvVariable("AI_MODEL_SUMMARY", ""),
	// Embedding model ID (overrides OPENROUTER_EMBEDDING_MODEL)
	AI_MODEL_EMBEDDING: getEnvVariable("AI_MODEL_EMBEDDING", ""),
